...
//            AND,OR 기호를 붙여 WHERE절 을 만들어야하는 경우 사용
...
// pageable에 설정한 페이지번호,출력개수,정렬을 적용하여 query(쿼리)문을 변경
    ...
    // 무한 순환 참조를 막기 위해 DTO로 변경
        ...
        PageImpl<ArticleListViewResponse> PageResponseDTO
                = new PageImpl<>(articleList, pageRequestDTO.getPageable(), count);
        PageResponseDTO<ArticleListViewResponse> response =
                new PageResponseDTO<>(pageRequestDTO,PageResponseDTO);
            return response;
    }
}
