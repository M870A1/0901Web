...
//  상속 방식 :  class 클래스 이름 extends 상속받을클래스{}
//  name,hp,mp 변수와 domove(),printUnit()메서드 를
//	선언하지 않아도 사용할 수 있다.
class Marine extends Unit {
	// 멤버 변수
	int attack;

// 자식 클래스의 생성자는 부모 클래스에 생성자가 있으면 반드시 실행해야 한다.
	Marine(String name, int hp, int mp, int attack) {
//		super(): 부모클래스의 생성자를 의미
		super(name, hp, mp);
//		marine 클래스의 멤버변수 attack를 설정
		this.attack = attack;
	}

	void printMarine() {
// 		 부모 클래스의 메서드는 자식클래스에서 자유롭게 사용할 수 있다.
		printUnit();
		System.out.println(", 공격력:" + attack);
//		부모 클래스의 멤버 변수는 자식클래스에서 자유롭게 사용할 수 있다.
...
//	*주의* 부모클래스에 생성자가 있다면 자식클래스에서 반드시 실행해야 한다.
		super(name, hp, mp);
		this.heal = heal;
	}
...
class Firebat extends Unit {
	...
	@Override // 오버라이드 된 메서드인것을 표시하는 문구
...
//		super.부모클래스의메서드() : 부모클래스의 메서드를 실행함.
	...
	void printFirebat() {
		printUnit();
		System.out.println(", 화염피해:" + fireattack);
		System.out.println(name + "," + hp + "," + mp + "," + fireattack);
	}
}
...
public class Ex01_Myterran {
	...
	public static void main(String[] args) {
		...
		Marine unit1 = new Marine("마린", 50, 0, 20);

		// name, hp, mp 세가지 변수는 Unit클래스를 상속 받아 사용
//	    attack 변수는 Marine클래스에서 선언하여 사용
		...
		Firebat unit3 = new Firebat("파이어뱃", 150, 0, 20);
		unit3.doMove();
		unit3.printFirebat();
	}
}
