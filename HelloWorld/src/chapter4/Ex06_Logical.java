...
//		논리 연산자 true 트루 false 펄스
//		&&(엔드 연산자) : true && true = true , true&&false=false , false&&false=false
		...
		int tall = 130;
		boolean ticket = true;
		System.out.println("콜러코스터 탑승 조건은 나이가 10살 이상,"
...
//		|| (Or연산자) : true && true = true , true&&false=true , false&&false=false
		...
		int 수학 = 80;
		int 영어 = 70;
		System.out.println("국어, 수학, 영어 중 90점이 넘는 과목이 있다면 합격입니다.");
		System.out.println("합격여부 : "+ (국어>=90 || 수학>=90 || 영어>=90));
		
//		비행기 탑승 가능 여부(And연산자 = 논리곱)
//		비행기를 타려면 여권,비행기표,9시 이전에 도착해야 합니다.	
		boolean 여권= false;
		boolean 비행기표 = true;
		int 현재시간 = 10;
		System.out.println("비행기에 타려면 여권,비행기표 를 소지하고 9시 이전에 도착해야 합니다.");
		System.out.println("비행기탑승 가능여부 : " +(여권 && 비행기표 && 현재시간<9));
	
//		논리 연산자 사용시의 주의점
//		최단 거리 평가(SCE)
//		1. And연산(&&) : 앞쪽 조건에서 false가 나오면 
//		               뒤의 조건은 계산하지 않고 결과를 false로 출력한다.
